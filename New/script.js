$(document).ready(function () {
    const API_URL = "https://us-central1-dashboard-4aa16.cloudfunctions.net/getUnifiedAkuResult"; 
    ...
    $('#theme-toggle').on('click', function() {
        ...
        $(this).html(isDark ? "<i class='bx bx-sun'></i>" : "<i class='bx bx-moon'></i>");
    });
    ...
    const Alert = Swal.mixin({
        customClass: { popup: 'sweetAlert' }, buttonsStyling: true, confirmButtonColor: 'var(--icon)'
    });
    ...
    async function loadStudents() {
        try {
            ...
            Alert.fire({ icon: 'error', title: 'Error', text: 'Failed to load student data.', confirmButtonText: 'OK' });
        }
    }

    $('.sem-btn').on('click', function () {
        $('.sem-btn').removeClass('active'); $(this).addClass('active');
    ...
    $('.type-btn').on('click', function () {
        ...
        if ($(this).attr('data-type') === 'list') {
            $('#view-list').show(); $('#view-reg').hide();
        } else {
            $('#view-list').hide(); $('#view-reg').show();
        }
        $('#result-wrapper').hide();
    });
    ...
    function renderStudents() {
        ...
        $listBody.empty(); $pagination.empty();
        
        const filtered = allStudents.filter(s => 
            s.name.toLowerCase().includes(currentFilter.toLowerCase()) || 
            s.regNumber.includes(currentFilter) || s.roll.includes(currentFilter)
        );

        if (filtered.length === 0) {
            $listBody.append(`<div class="table-row"><div class="table-cell" style="flex:1;">No students found</div></div>`);
            return;
        }
        ...
        filtered.slice(startIdx, startIdx + itemsPerPage).forEach(s => {
            ...
            const bgStyle = isLeft ? 'background-color: rgba(214, 48, 49, 0.15); color: var(--logout); font-weight: bold;' : '';
            ...
            $row.on('click', function() {
                if (isLeft) {
                    Alert.fire({ icon: 'error', title: 'Left The Course', confirmButtonText: 'OK' });
                } else {
        ...
        if (totalPages > 1) {
            for (let i = 1; i <= totalPages; i++) {
                ...
                $btn.on('click', function() { currentPage = i; renderStudents(); });
    ...
    $('#list-search').on('input', function() {
        currentFilter = $(this).val().trim(); currentPage = 1; renderStudents();
    });

    $('#btn-search-reg').on('click', function() {
        const reg = $('#reg-input').val().trim();
        if (reg.length > 5) fetchResult(reg);
        else Alert.fire({ icon: 'warning', title: 'Invalid Reg. Number', confirmButtonText: 'OK' });
    });

    $('#reg-input').on('keypress', function(e) { if(e.which == 13) $('#btn-search-reg').click(); });

    // --- SKELETON LOADER ---
    function displaySkeletonUI() {
        $('#result-wrapper').show();
        
        // This exact styling guarantees the skeleton shows up in the Student Details section
    ...
    async function fetchResult(regNumber) {
        if (!currentSem) return Alert.fire({ icon: 'warning', title: 'Select a Semester first!', confirmButtonText: 'OK' });
        
        displaySkeletonUI();
        $('html, body').animate({ scrollTop: $("#result-wrapper").offset().top - 20 }, 400);

        try {
            const res = await fetch(`${API_URL}?s=${currentSem}&reg=${regNumber}`);
            ...
            setTimeout(() => { renderResultUI(data); }, 300); 
        } catch (err) {
            $('#result-wrapper').hide(); 
            Alert.fire({ icon: 'error', title: 'Failed', text: err.message, confirmButtonText: 'OK' });
        }
    }
    ...
    function renderResultUI(data) {
        ...
        const buildRow = (sub) => {
            const isFail = ["F", "AB", "FAIL"].includes(sub.grade ? sub.grade.trim().toUpperCase() : "-");
            const cleanName = sub.name.replace(/^PHARMA/i, "").trim() || "Unknown";
            return `
                <div class="table-row noselect">
        ...
        if (data.theory && data.theory.length > 0) {
            $('#theory-container').show(); $('#theory-body').html(data.theory.map(buildRow).join(""));
        } else $('#theory-container').hide();

        if (data.practical && data.practical.length > 0) {
            $('#practical-container').show(); $('#practical-body').html(data.practical.map(buildRow).join(""));
        } else $('#practical-container').hide();

        const failedSubjects = [...(data.theory||[]), ...(data.practical||[])]
            .filter(sub => ["F", "AB", "FAIL"].includes(sub.grade ? sub.grade.trim().toUpperCase() : "-"))
            .map(sub => sub.code);

        if (failedSubjects.length > 0) {
            $('#res-remarks').html(`<div style="color: var(--logout); font-size: 11px; font-weight: 800; margin-bottom: 5px;">FAIL : ${failedSubjects.join(", ")}</div><div style="font-size: 11px; font-weight: 900; color: var(--text);">SGPA : ${data.sgpa}</div>`);
        } else {
            $('#res-remarks').html(`<div style="font-size: 11px; font-weight: 900; color: var(--text);">SGPA : ${data.sgpa}</div>`);
        }
    }
